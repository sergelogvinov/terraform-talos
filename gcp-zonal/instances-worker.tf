...
resource "google_compute_instance_group_manager" "worker_spot" {
  for_each = { for k, v in var.instances : k => v if k != "all" }

  name               = "${local.cluster_name}-worker-as-${each.key}-mig"
  description        = "${local.cluster_name}-worker autoscaling spot group"
  project            = local.project
  zone               = each.key
  base_instance_name = "worker-${each.key}"

  version {
    instance_template = google_compute_instance_template.worker_spot[each.key].id
  }
  update_policy {
    type                  = "OPPORTUNISTIC"
...
locals {
  ...
  worker_autoscaller = "os=linux;os_distribution=cos;arch=amd64;kube_reserved=cpu=50m,memory=128Mi,ephemeral-storage=1Gi;evictionHard=cpu=50m,memory=64Mi"
}
...
resource "google_compute_instance_template" "worker" {
  ...
  metadata = {
    ...
    kube-env         = "AUTOSCALER_ENV_VARS: node_labels=${local.worker_labels};${local.worker_autoscaller}"
...
resource "google_compute_instance_template" "worker_spot" {
  ...
  name_prefix  = "${local.cluster_name}-worker-spot-${each.key}-"
  description  = "${local.cluster_name}-worker spot template"
  project      = local.project
  region       = local.region
  machine_type = "n2d-standard-2" # lookup(each.value, "worker_type", "n2d-standard-2")
